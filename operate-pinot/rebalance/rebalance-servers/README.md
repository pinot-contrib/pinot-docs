...
| diskUtilization | Check whether disk utilization could exceed `controller.rebalance.disk.utilization.threshold` (default `0.9`) during or after the rebalance. This check considers both the transient footprint while segments move and the final footprint after unassigned segments are removed. | PASS when both footprints remain within the threshold. It also passes when only the transient footprint exceeds the threshold and `lowDiskMode=true, downtime=false`, because segments are removed before replacements are added. ERROR when the final footprint exceeds the threshold, or when the transient footprint exceeds it without an effective low-disk mode. WARN if disk usage information is not yet available. |
...
| rebalanceConfigOptions | Mark any parameters in the rebalance that need a double check, as they might cause performance impact. This includes `lowDiskMode=true` with `downtime=true`, because low-disk mode has no effect during a downtime rebalance. | PASS if no rebalance parameter needs a double-check. WARN if any rebalance parameter that is flagged is set, followed by the description. |
