...
| includeTables | `[]` | Comma separated list of tables with type that should be included in this tenant rebalance job. Leaving blank defaults to include all tables from the tenant. Example: "table1_REALTIME, table2_REALTIME" Notice that a table will not be included if it doesn't belong to the tenant. |
