...
| --- | --- | --- | --- |
| ``pinot.server.max.segment.preprocess.parallelism `` | 0 <= value <= Integer.MAX_VALUE | Integer.MAX_VALUE | The maximum parallelism to perform index rebuild operations on a segment across all indexes **after** the server is ready to serve queries. Setting this to `0` acts as a kill switch and blocks new preprocess work until the value is raised again. |
| ``pinot.server.max.segment.preprocess.parallelism.before.serving.queries `` | 0 <= value <= Integer.MAX_VALUE | Integer.MAX_VALUE | The maximum parallelism to perform index rebuild operations on a segment across all indexes **before** the server is ready to serve queries (start up). Setting this to `0` acts as a kill switch and blocks new preprocess work until the value is raised again. |
| ``pinot.server.max.segment.startree.preprocess.parallelism `` | 0 <= value <= Integer.MAX_VALUE | Integer.MAX_VALUE | The maximum parallelism to perform star-tree index rebuild operations on a segment **after** the server is ready to serve queries. Star-tree index rebuild can be more resource intensive than other index operations. Setting this to `0` acts as a kill switch and blocks new start-tree preprocess work until the value is raised again. |
| ``pinot.server.max.segment.startree.preprocess.parallelism.before.serving.queries `` | 0 <= value <= Integer.MAX_VALUE | Integer.MAX_VALUE | The maximum parallelism to perform star-tree index rebuild operations on a segment **before** the server is ready to serve queries (start up). Star-tree index rebuild can be more resource intensive than other index operations. Setting this to `0` acts as a kill switch and blocks new start-tree preprocess work until the value is raised again. |
| ``pinot.server.max.segment.download.parallelism `` | 0 <= value <= Integer.MAX_VALUE | Integer.MAX_VALUE | The maximum parallelism to download and untar segments from deep store or peer servers **after** the server is ready to serve queries. Setting this to `0` acts as a kill switch and blocks new downloads until the value is raised again. |
| ``pinot.server.max.segment.download.parallelism.before.serving.queries `` | 0 <= value <= Integer.MAX_VALUE | Integer.MAX_VALUE | The maximum parallelism to download and untar segments from deep store or peer servers **before** the server is ready to serve queries (start up). Setting this to `0` acts as a kill switch and blocks new downloads until the value is raised again. |
