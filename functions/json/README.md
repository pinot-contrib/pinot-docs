...
| Function                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [**JSONEXTRACTSCALAR**](jsonextractscalar.md)`(jsonField, 'jsonPath', 'resultsType', [defaultValue])` Evaluates the `'jsonPath'` on `jsonField`, returns the result as the type `'resultsType'`, use optional `defaultValue`for null or parsing error. |
| [**JSONEXTRACTSCALARFAST**](jsonextractscalar.md#jsonextractscalarfast)`(jsonField, 'jsonPath', 'resultsType', [defaultValue])` Uses streaming extraction for simple linear paths, preserving ordinary `JSONEXTRACTSCALAR` behavior and falling back when needed. |
| [**JSONEXTRACTSCALARFIRSTMATCH**](jsonextractscalar.md#jsonextractscalarfirstmatch)`(jsonField, 'jsonPath', 'resultsType', [defaultValue])` Stops after the first addressed value. Use only with well-formed, duplicate-free JSON. |
| [**JSONEXTRACTSCALARFORY**](jsonextractscalar.md#jsonextractscalarfory-experimental)`(jsonField, 'jsonPath', 'resultsType', [defaultValue])` Experimental Apache Fory variant; requires optional Fory runtime dependencies and otherwise uses the compatibility fallback. |
| [**JSONEXTRACTKEY**](jsonextractkey.md)`(jsonField, 'jsonPath', ['paramString'])` Extracts all matched JSON field keys based on `'jsonPath'` into a `STRING_ARRAY.` |
| [**JSONEXTRACTINDEX**](jsonextractindex.md)`(jsonField, 'jsonPath', index, 'resultsType', [defaultValue])` Extracts the indexed value from an array matched by `'jsonPath'` and returns it as the requested scalar type. |
...
| ​[**JSONEXTRACTOBJECT**(jsonField)](../../functions/json/jsonextractobject.md)                                | Parses a JSON document once into a reusable object or array for downstream `JSONPATH*` extraction. Returns `null` for `null`, scalar, or non-JSON input without throwing.                                                                                                                                                                                                                                                         |
...
| ​[**JSONPATHLONGFAST**(jsonField, 'jsonPath', \[defaultValue\])](../../functions/json/jsonpathlong.md#fast-and-first-match-variants)          | Opt-in fast variant of `JSONPATHLONG` for simple linear paths. Keeps the same result as `JSONPATHLONG` and falls back to it for unsupported JsonPath shapes or non-JSON input.                                                                                                                                                                                                                                                   |
| ​[**JSONPATHLONGFIRSTMATCH**(jsonField, 'jsonPath', \[defaultValue\])](../../functions/json/jsonpathlong.md#fast-and-first-match-variants)          | Early-exit variant of `JSONPATHLONGFAST`. Use only with well-formed, duplicate-free JSON because duplicate keys resolve to the first occurrence and malformed content after the addressed field is not rejected.                                                                                                                                                                                                                  |
...
| ​[**JSONPATHDOUBLEFAST**(jsonField, 'jsonPath', \[defaultValue\])](../../functions/json/jsonpathdouble.md#fast-and-first-match-variants)      | Opt-in fast variant of `JSONPATHDOUBLE` for simple linear paths. Keeps the same result as `JSONPATHDOUBLE` and falls back to it for unsupported JsonPath shapes or non-JSON input.                                                                                                                                                                                                                                               |
| ​[**JSONPATHDOUBLEFIRSTMATCH**(jsonField, 'jsonPath', \[defaultValue\])](../../functions/json/jsonpathdouble.md#fast-and-first-match-variants)      | Early-exit variant of `JSONPATHDOUBLEFAST`. Use only with well-formed, duplicate-free JSON because duplicate keys resolve to the first occurrence and malformed content after the addressed field is not rejected.                                                                                                                                                                                                                |
...
| ​[**JSONPATHSTRINGFAST**(jsonField, 'jsonPath', \[defaultValue\])](../../functions/json/jsonpathstring.md#fast-and-first-match-variants)      | Opt-in fast variant of `JSONPATHSTRING` for simple linear paths. Keeps the same result as `JSONPATHSTRING` and falls back to it for unsupported JsonPath shapes or non-JSON input.                                                                                                                                                                                                                                               |
| ​[**JSONPATHSTRINGFIRSTMATCH**(jsonField, 'jsonPath', \[defaultValue\])](../../functions/json/jsonpathstring.md#fast-and-first-match-variants)      | Early-exit variant of `JSONPATHSTRINGFAST`. Use only with well-formed, duplicate-free JSON because duplicate keys resolve to the first occurrence and malformed content after the addressed field is not rejected.                                                                                                                                                                                                                |
...
| [**JSONKEYVALUEARRAYTOMAP**](jsonkeyvaluearraytomap.md)`(keyValueArray, [keyColumnName], [valueColumnName])` | Extracts an array of key-value objects into a map. Default `keyColumnName` is `key` and default `valueColumnName` is `value`. |
...
| [JSONPATHSTRING](jsonpathstring.md) | [JSONKEYVALUEARRAYTOMAP](jsonkeyvaluearraytomap.md) |
| [JSONEXTRACTOBJECT](jsonextractobject.md) | [JSONPATHEXISTS](jsonpathexists.md) |
