...
| JSON | `JSONEXTRACTSCALAR`, `JSONEXTRACTOBJECT`, `JSONPATHSTRING`, `JSONPATHLONG`, `JSONPATHDOUBLE`, `JSONFORMAT`, `TOJSONMAPSTR` | Typed query output and ingestion configs | Prefer the typed helpers when the output column needs a stable SQL type, and use `JSONEXTRACTOBJECT` when you want to parse once in ingestion and reuse the result. |
