...
| streamConfigMaps                                     | Set of key-value pairs, where the keys are the names of the Kafka topics, and the values are objects that contain additional configuration information for each topic. For example, specify the schema of the data in the Kafka topic or the name of the Pinot table to map the topic to.                                            | String                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| topic.consumption.rate.limit                         | Determines the upper bound for consumption rate for the whole topic. Having a consumption rate limiter is beneficial in case the stream message rate has a bursty pattern which leads to long GC pauses on the Pinot servers. The rate limiter can also be considered as a safeguard against excessive ingestion of realtime tables. | Double. The values should be greater than zero.                                                                                                                                                                                                                                                                                                                                                                                                           |
