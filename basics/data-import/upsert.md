...
To prevent this, we recommend using explicit [partitioned replica-group instance assignment](../../for-operators/operating-pinot/instance-assignment.md#partitioned-replica-group-instance-assignment) to ensure the instance assignment is persisted. Note that `numInstancesPerPartition` should always be `1` in `replicaGroupPartitionConfig`.
{% endhint %}
...
![Disable the upsert during query via query option](https://github.com/pinot-contrib/pinot-docs/blob/latest/disable\_upsert\_during\_query.png)
