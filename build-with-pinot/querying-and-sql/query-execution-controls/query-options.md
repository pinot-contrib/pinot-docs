...
| **maxExecutionThreads** | Maximum threads to use to execute the query. Useful to limit the resource usage for expensive queries. When the query does not set this option, Pinot first checks `pinot.server.query.executor.default.execution.threads`; if that is unset or non-positive, Pinot falls back to `pinot.server.query.executor.max.execution.threads`. | Use the query value when set. Otherwise use the server default if configured, or fall back to Pinot's legacy automatic thread selection behind `pinot.server.query.executor.max.execution.threads`. |
...
| **useFixedReplica** | Route each segment to a consistent replica for the query instead of relying on the broker's normal per-query server selection. This overrides the table-level or broker-level fixed-replica setting for the query. When replica-group based routing is enabled, Pinot still honors `numReplicaGroupsToQuery`; fixed-replica routing only makes the replica choice deterministic within each queried replica-group. | Use the table-level or broker-level fixed-replica setting; when neither is set, `false` |
...
| **numGroupsLimit** | For group-by queries, cap how many groups each Pinot query operator keeps before it stops admitting new groups. This applies to both leaf and intermediate stages in the multi-stage engine. When the limit is reached, Pinot keeps aggregating rows for groups it has already seen and sets `numGroupsLimitReached=true` in the response metadata. See [Grouping algorithm](../grouping-algorithm.md) for the related server config and trimming behavior. | Server level config (default `100000`) |
| **numGroupsWarningLimit** | For group-by queries, set the warning threshold for the number of groups a query operator accumulates. When the threshold is reached, Pinot logs a warning and sets `numGroupsWarningLimitReached=true` in the response metadata, but continues query execution. | Server level config (default `150000`) |
| **errorOnNumGroupsLimit** | For multi-stage group-by queries, throw an exception instead of returning partial results when `numGroupsLimit` or the aggregate hint `num_groups_limit` is reached. The aggregate hint `error_on_num_groups_limit='true'` provides the same behavior for a specific aggregate node. | `false` |
...
| **minInitialIndexedTableCapacity** | For group-by queries, set the minimum initial capacity Pinot uses when creating `IndexedTable` instances to merge grouped results. Higher values can reduce rehashing for queries with many groups, but increase memory usage for smaller queries. This query option overrides both the server-side and broker-side defaults. | `128` |
...
| **vectorNprobe** | For IVF_FLAT, IVF_PQ, and IVF_ON_DISK vector search, set how many inverted lists Pinot probes per segment before collecting ANN candidates. Higher values usually improve recall at the cost of latency. Pinot ignores this option for HNSW indexes and for the exact-scan fallback path used on segments without a vector index. See [Vector index](../../indexing/vector-index.md) for the related index configuration. | `4` for IVF_FLAT, IVF_PQ, and IVF_ON_DISK; ignored for HNSW and exact-scan fallback |
| **vectorExactRerank** | For vector search that uses an ANN index, re-score the ANN candidates with exact distance from the forward index before Pinot returns the final top-K. This improves accuracy but does not turn ANN search into a full exact scan. See [Vector index](../../indexing/vector-index.md) for the related ANN search behavior. | `true` for IVF_PQ (PQ distances are approximate by construction); `false` for HNSW and IVF_FLAT |
| **vectorMaxCandidates** | When `vectorExactRerank=true`, set how many ANN candidates Pinot retrieves before exact reranking. Pinot always uses at least `topK`, and ignores this option when exact rerank is disabled. See [Vector index](../../indexing/vector-index.md) for tuning guidance. | `topK * 10` when exact rerank is enabled |
| **vectorDistanceThreshold** | For vector similarity queries, return all results within this distance threshold instead of limiting to top-K. This enables confidence-based retrieval where all results meeting a quality threshold are returned. Applies to `VECTOR_SIMILARITY` queries; the distance function used in the WHERE clause determines which threshold is applied. See [Vector Query Execution Semantics](../vector-query-execution.md) for usage examples. | Not set (uses top-K retrieval) |
| **vectorEfSearch** | For HNSW vector search, control how many nodes the graph traversal visits (search beam width). Higher values improve recall at the cost of query latency. Typical ranges: 100–150 for low latency, 200–300 for balanced, 400–800 for high recall. Pinot ignores this option for IVF indexes and the exact-scan fallback path. Applies to both mutable (consuming) and immutable (offline) HNSW segments. See [Vector index](../../indexing/vector-index.md) for tuning guidance. | From index config (defaults to `topK` when not set) |
| **vectorUseRelativeDistance** | For HNSW vector search, toggle competitive (relative-distance) pruning during graph traversal. Disabling this (`false`) can improve recall on some data distributions at the cost of higher latency. Pinot ignores this option for non-HNSW indexes. See [Vector index](../../indexing/vector-index.md) for tuning guidance. | `true` |
| **vectorUseBoundedQueue** | For HNSW vector search, toggle the bounded top-K collector during graph traversal. Disabling this (`false`) may improve recall in certain scenarios. Pinot ignores this option for non-HNSW indexes. See [Vector index](../../indexing/vector-index.md) for tuning guidance. | `true` |
...
| **usePlannerRules** | Which query planner rules that are disabled by default should be used. This is useful when the defaultly disabled rules could help query execution. Config can be specified using rule names delimited by comma: `usePlannerRules='AggregateJoinTransposeExtended,SortProjectTranspose'`. By default this query option applies to the built-in disabled-rule set listed in [Default Disabled Rules](../default-disabled-rules.md), but brokers can replace that set with `pinot.broker.mse.planner.disabled.rules`. If a broker replaces the built-in set and you still want a rule such as `SortProjectTranspose` to remain opt-in, include it in the broker-configured disabled list as well. At this point, Pinot does not have a cost-based optimizer and the multi-stage query engine uses Calcite's HepPlanner for query optimization. The rules that are disabled by default are those that are only helpful under certain circumstances. For a more detailed description on what these rules do and when they are helpful, please see [Default Disabled Rules](../default-disabled-rules.md). | `null/empty` (no defaultly disabled rules are used) |
...
| **inPredicatePruningThreshold** | Override the maximum number of values in an `IN` predicate for which Pinot attempts value-based or bloom-filter segment pruning. Set a larger positive value to permit pruning for larger lists, or a negative value to always attempt pruning regardless of list size. This controls whole-segment pruning only: a pruner either drops a segment that contains none of the values or keeps the complete segment; row filtering still happens during query execution. | Server `inpredicate.threshold` setting (default `10`) |
...
| **filteredAggregationsSkipEmptyGroups** | Set to `true` to avoid computing groups that match the main query filter but none of the aggregation-specific `FILTER (WHERE ...)` clauses. This applies to filtered aggregations referenced only by `HAVING` or `ORDER BY` as well as those projected in `SELECT`. By default, Pinot computes all groups over the rows returned by the main filter, which is standard SQL behavior. Enabling this option can improve performance when empty groups are not required and the aggregation filters are much more selective than the main filter. For example, `SELECT groupKey, COUNT(*) FROM myTable GROUP BY groupKey HAVING SUM(X) FILTER (WHERE Y = 1) > 0 OPTION(filteredAggregationsSkipEmptyGroups=true)` can use the filtered execution path without materializing groups that have no matching `Y = 1` row. The corrected single-stage behavior for filtered aggregations used only in `HAVING` or `ORDER BY` may change results from older releases that silently evaluated those aggregations without their filters; see [Apache Pinot #19437](https://github.com/apache/pinot/pull/19437). | `false` (all groups are computed by default as required by standard SQL) |
...
| **optimizeMaxInitialResultHolderCapacity** | For single-stage `GROUP BY`, use equality and `IN` predicates on single-value grouping columns to reduce the initial result-holder capacity. Pinot still selects the group-key holder from the columns' full cardinality product so sparse dictionary IDs and numeric overflow remain correct. Predicates on multi-value grouping columns do not reduce the bound because every value in a matching row contributes a group. | `false` |
...
| **usePhysicalOptimizer** | Enable the Physical Optimizer for the multi-stage engine (MSE). The Physical Optimizer can automatically eliminate or simplify redundant Exchanges (shuffles) for arbitrarily complex queries without requiring query hints. Must be used with `useMultistageEngine=true`. See [physical-optimizer.md](../multi-stage-query/physical-optimizer.md) for details. (introduced in 1.4.0) | `false` (disabled) |
| **unnestColumnPruning** | For multi-stage `UNNEST` queries on the logical planner path, prune unused passthrough columns from the `UNNEST` output, including the source array when nothing downstream references it. This can reduce row width and exchange payload size, but it makes the broker emit a smaller `UNNEST` output schema, so only enable it after all servers in the cluster support the feature. Pinot ignores this option when `usePhysicalOptimizer=true`. | Broker level config (default `false`) |
| **sortExchangeCopyThreshold** | For multi-stage `ORDER BY ... LIMIT` queries, control when Pinot pushes the sort-and-limit operation below the sort exchange so each upstream worker sends only its top rows. Pinot applies the rewrite only when the effective fetch is at or below this integer threshold. If there is no `LIMIT`, or the effective fetch is larger than the threshold, Pinot keeps the normal sort exchange plan. | Broker level config (default `10000`) |
| **inferRealtimeSegmentPartition** | For partitioned realtime tables in the multi-stage engine, infer segment partitions from realtime segment names when Pinot cannot rely on the stored segment-partition metadata. Pinot still uses the table's `segmentPartitionConfig` for the partition column, function, and partition count. If Pinot cannot infer partitions for the routed realtime segments, it falls back to unpartitioned distribution. | `false` |
| **useLiteMode** | Enable Multistage Engine Lite Mode, which runs MSE queries using a scatter-gather paradigm (like the single-stage engine) with a configurable limit on rows returned by each leaf stage instance (default 100k). This allows safe access to MSE features like window functions, subqueries, and joins at high QPS with minimal reliability risks. Requires both `useMultistageEngine=true` and `usePhysicalOptimizer=true`. See [multistage-lite-mode.md](../multi-stage-query/multistage-lite-mode.md) for details. (introduced in 1.4.0) | `false` (disabled) |
| **liteModeLeafStageLimit** | For Multistage Engine Lite Mode, override the maximum number of records a leaf stage worker is allowed to return. The value must be a positive integer. See [multistage-lite-mode.md](../multi-stage-query/multistage-lite-mode.md) for Lite Mode behavior. | Broker level config (default `100000`) |
| **liteModeLeafStageFanOutAdjustedLimit** | For Multistage Engine Lite Mode, override the fan-out-adjusted leaf-stage limit with a positive integer. Pinot divides this value by the number of workers assigned to the leaf stage and uses the quotient as the per-worker hard limit instead of `liteModeLeafStageLimit`. See [multistage-lite-mode.md](../multi-stage-query/multistage-lite-mode.md) for Lite Mode behavior. | Broker level config (disabled when unset or non-positive) |
| **runInBroker** | For Multistage Engine Lite Mode, control where Pinot runs the non-leaf stages. When `true`, Pinot runs the non-leaf stages in the broker to preserve the scatter-gather execution model. When `false`, Pinot runs the non-leaf stages on servers instead. See [multistage-lite-mode.md](../multi-stage-query/multistage-lite-mode.md) for details. | Broker level config (default `true` when Lite Mode is enabled) |
| **useLeafServerForIntermediateStage** | For multi-stage queries, use the same servers chosen for leaf stages as the workers for intermediate stages instead of selecting from all enabled servers. This helps control fanout and reduce data shuffling. If every non-replicated leaf stage is empty because all segments were pruned or the table has no data, Pinot short-circuits and runs only the broker reduce stage locally instead of dispatching an intermediate stage to all enabled servers. | `false` |
| **streamStats** | For multi-stage queries, switch broker-to-server dispatch to the streaming `SubmitWithStream` stats path instead of the legacy unary submit path. This makes per-operator stage stats more reliable on error paths and adds `streamStatsCoverage` to the response so you can see whether each stage reported completely. Only enable this after every server in the cluster supports the streaming RPC because Pinot does not fall back automatically on mixed-version clusters. | Broker level config (default `false`) |
| **inferPartitionHint** | For multi-stage joins, tell Pinot to infer partition information from the joined tables so it can use colocated execution when the table partitioning and server assignment allow it. This overrides the broker-level `pinot.broker.multistage.infer.partition.hint` setting for the query. If a scan already has a partial `tableOptions` hint, explicit `partition_key` and `partition_function` values override the inferred values and are validated against the table metadata; scans hinted with `tableOptions(is_replicated='true')` are left untouched. See [Colocated join strategy](../multi-stage-query/join-strategies/colocated-join-strategy.md) for the conditions and alternatives such as explicit `tableOptions` hints. | Broker level config (default `false`) |
...
| **workloadName** | Assigns the query to a named workload for CPU and memory accounting and workload budget enforcement. This works with the host-side `accounting.workload.*` controls, and when the `workload` scheduler is enabled the same option also selects the scheduler-managed workload. See [Workload-Based Query Resource Isolation](../../../operate-pinot/tuning/workload-query-isolation.md) (introduced in 1.4.0) | `null/empty` (query belongs to the default workload with no budget enforcement) |
...
| **mseMinGroupTrimSize** | For multi-stage group-by queries, set how many groups Pinot keeps when trimming intermediate-stage aggregation results. This provides the same override as the `mse_min_group_trim_size` aggregate hint described in [Grouping algorithm](../grouping-algorithm.md#configuration-parameters). | Server level config (default `5000`) |
...
| Key | Description | Default Behavior |
| --- | --- | --- |
| **useBrokerPruning** | When set to `true`, enables broker-side segment pruning for multi-stage leaf-stage routing. On the physical optimizer path, Pinot uses the broker default from `pinot.broker.multistage.use.broker.pruning`. On the logical planner path, Pinot uses the broker default from `pinot.broker.multistage.logical.planner.use.broker.pruning` for eligible non-partitioned leaves, partitioned leaves, logical tables, and colocated joins. In a filtered colocated join, a partition class is removed only when every member of the group prunes it. If Pinot cannot route the filter-bearing pruning request, it retries the query with unpruned routing instead of failing it. Query-level `useBrokerPruning` overrides the applicable broker default. | Physical optimizer path: broker config (default `true`); logical planner eligible leaf stages: broker config (default `true`) |

### Index-Based DISTINCT

| Key | Description | Default Behavior |
| --- | --- | --- |
| **useIndexBasedDistinctOperator** | When `true`, enables index-based DISTINCT operators when applicable. Pinot routes eligible single-column `SELECT DISTINCT jsonExtractIndex(col, path, type[, defaultValue[, jsonFilterExpression]])` queries to `JsonIndexDistinctOperator`, which reads distinct values directly from the JSON index instead of projecting every matching document. The queried JSON path must have a backing JSON index. Supported DISTINCT result types are `INT`, `LONG`, `FLOAT`, `DOUBLE`, `BIG_DECIMAL`, and `STRING`, plus their `_ARRAY` forms; `_ARRAY` results are flattened into scalar DISTINCT rows. Pinot also routes eligible single-column `SELECT DISTINCT col` queries on dictionary-encoded columns with an inverted index to `InvertedIndexDistinctOperator`. | `false` (disabled) |
