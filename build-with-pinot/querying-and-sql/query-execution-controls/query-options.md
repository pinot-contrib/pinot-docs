...
| **vectorNprobe** | For IVF_FLAT vector search, set how many inverted lists Pinot probes per segment before collecting ANN candidates. Higher values usually improve recall at the cost of latency. Pinot ignores this option for HNSW indexes and for the exact-scan fallback path used on segments without a vector index. See [Vector index](../../indexing/vector-index.md) for the related index configuration. | `4` for IVF_FLAT; ignored for other vector execution paths |
| **vectorExactRerank** | For vector search that uses an ANN index, re-score the ANN candidates with exact distance from the forward index before Pinot returns the final top-K. This improves accuracy but does not turn ANN search into a full exact scan. See [Vector index](../../indexing/vector-index.md) for the related ANN search behavior. | `false` |
