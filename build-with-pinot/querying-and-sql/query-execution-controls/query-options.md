...
| **inferPartitionHint** | For multi-stage joins, tell Pinot to infer partition information from the joined tables so it can use colocated execution when the table partitioning and server assignment allow it. This overrides the broker-level `pinot.broker.multistage.infer.partition.hint` setting for the query. See [Colocated join strategy](../multi-stage-query/join-strategies/colocated-join-strategy.md) for the conditions and alternatives such as explicit `tableOptions` hints. | Broker level config (default `false`) |
