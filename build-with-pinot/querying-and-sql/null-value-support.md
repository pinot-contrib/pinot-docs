...
Filters also use [three-valued SQL logic](sql-reference.md#three-valued-logic): a null row is neither a match for a comparison on that column nor a match for its negation. This applies to indexed filter shortcuts and to the fast filtered count, not just to rows evaluated one at a time. For example, with `enableNullHandling=true`, both `WHERE c <> 1` and `WHERE NOT (c = 1)` exclude rows where `c` is null, even if the stored default null value would satisfy the comparison. `COUNT(*)` still counts every row *selected by the filter*; it does not count excluded null rows. Use `WHERE c IS NULL` to select those rows explicitly. A star-tree index cannot serve a filtered query when a filtered column contains null rows in that segment.
