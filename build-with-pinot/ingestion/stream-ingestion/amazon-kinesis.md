...
| requests\_per\_second\_limit | Controls the maximum number of `getRecords` requests per second that the consumer will make to a Kinesis shard. This parameter is crucial for avoiding AWS Kinesis API throttling. Kinesis enforces a hard limit of 5 `getRecords` requests per second per shard. Exceeding this limit results in `ProvisionedThroughputExceededException`. The default value of 1 is intentionally conservative to prevent throttling in replicated setups where multiple consumers might be reading from the same shard simultaneously. You should only increase this if you are experiencing slow consumption rates and do not have `ProvisionedThroughputExceededException` in the logs yet |
