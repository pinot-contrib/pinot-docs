...
| useApproximateFunction     | Whether to rewrite eligible exact distinct-count and percentile aggregates to SMART variants for this table in the single-stage engine. A query-level `useApproximateFunction` option overrides this value; this value overrides the cluster and broker `pinot.broker.use.approximate.function` defaults. Multi-stage queries can span tables, so this table setting does not apply to them. See [Approximate aggregate guardrail](cluster.md#approximate-aggregate-guardrail). |
...
| skipSegmentPreprocess                      | Boolean to skip the segment preprocessing step on every server load path, including startup, reload, cold download, CRC-mismatch replacement, and consuming-segment replacement. When set to `true`, the server will not check or update default columns, single-column indexes (inverted, JSON, etc.), star-tree indexes, multi-column text indexes, or column min/max values when loading segments. This can reduce the time before freshly downloaded segments become online as well as startup and reload time. Default: `false`. This property can be overridden per tier through `tableIndexConfig.tierOverwrites`, for example to skip preprocessing on a stable hot tier but enable it on a cold tier. **Use with caution**: enabling this means any new indexes or schema changes that require segment preprocessing will not be applied until this flag is set back to `false` and segments are reloaded. Only recommended for production tables with stable schemas and index configurations where faster availability is more important than automatic index updates. |
