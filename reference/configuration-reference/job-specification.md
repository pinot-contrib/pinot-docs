...
| includeFileNamePattern | Include only files whose full path matches this Java NIO `PathMatcher` pattern. Supports both the `glob:` and `regex:` prefixes. See [File name patterns](job-specification.md#file-name-patterns). |
| excludeFileNamePattern   | Exclude files whose full path matches this Java NIO `PathMatcher` pattern. Supports both the `glob:` and `regex:` prefixes. See [File name patterns](job-specification.md#file-name-patterns). |
...
Pinot passes `includeFileNamePattern` and `excludeFileNamePattern` to Java's `FileSystems.getDefault().getPathMatcher(...)`. Both properties require one of these syntax prefixes:

* `glob:` uses Java NIO glob syntax.
* `regex:` uses Java regular expression syntax, not PCRE or JavaScript regular expression syntax.

The matcher is applied to the whole path returned by the configured `PinotFS`, not only the final file name. For example, these patterns include Avro files and exclude temporary files at any depth:
...
The equivalent suffix filters can use Java regular expressions:

```yaml
includeFileNamePattern: 'regex:.*[.]avro'
excludeFileNamePattern: 'regex:.*[.]tmp'
```

Job specs are rendered with Groovy `SimpleTemplateEngine` before YAML or JSON parsing. A `$` can therefore be interpreted as template syntax, while backslashes can require escaping for both the template and the job-spec format. When possible, avoid fragile escaping. For example, use `[.]parquet` to match a literal dot and let whole-path matching provide the end condition:

```yaml
includeFileNamePattern: 'regex:.*[.]parquet'
```

Paths are also normalized by the default Java file system before matching. For example, `Paths.get("s3://bucket/path/file.parquet").toString()` produces `s3:/bucket/path/file.parquet`. Match the normalized path, with one slash after the scheme:

```yaml
includeFileNamePattern: 'regex:^s3:/bucket/path/.*[.]parquet'
```

See the Java documentation for [`FileSystem.getPathMatcher`](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/nio/file/FileSystem.html#getPathMatcher(java.lang.String)) for the complete syntax.
...
| field           | description                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| dataFormat      | Record data format, e.g. 'avro', 'parquet', 'orc', 'csv', 'json', 'bson', 'thrift' etc.                                                                                                                                                                                                                                                                                                                                                                  |
| className | Corresponding RecordReader class name. E.g. org.apache.pinot.plugin.inputformat.avro.AvroRecordReader org.apache.pinot.plugin.inputformat.csv.CSVRecordReader org.apache.pinot.plugin.inputformat.parquet.ParquetRecordReader org.apache.pinot.plugin.inputformat.json.JSONRecordReader org.apache.pinot.plugin.inputformat.bson.BSONRecordReader org.apache.pinot.plugin.inputformat.orc.ORCRecordReader org.apache.pinot.plugin.inputformat.thrift.ThriftRecordReader |
| configClassName | Corresponding RecordReaderConfig class name, it's mandatory for CSV and Thrift file format. E.g. org.apache.pinot.plugin.inputformat.csv.CSVRecordReaderConfig org.apache.pinot.plugin.inputformat.thrift.ThriftRecordReaderConfig. BSON does not require a config class for the default reader. |
| configs         | Used to init RecordReaderConfig class name, this config is required for CSV and Thrift data format. Most BSON ingestion uses only `dataFormat: bson` and the default `BSONRecordReader`.                                                                                                                                                                                                                  |
