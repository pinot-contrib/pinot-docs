...
| pinot.server.protobuf.descriptor.fallback.enabled | true | For remote descriptor files used by `ProtoBufMessageDecoder`, allow a server to reuse its last successfully fetched and resolved descriptor for the same URI when a later fetch fails. Set to `false` to fail decoder creation on fetch errors. Updates reach running servers without a restart and are consulted at decoder creation; removing the key restores `true`. A table's `stream.[streamType].decoder.prop.descriptorFileFallbackEnabled` overrides this setting. See [Kafka Protobuf ingestion](../../build-with-pinot/ingestion/stream-ingestion/import-from-apache-kafka.md#use-protocol-buffers-protobuf-format). |
...
| pinot.broker.query.option.validationMode | NONE | Live cluster policy for unknown query-option keys in SQL `SET` or `OPTION(...)` on data queries: `NONE` preserves them, `WARN` preserves and logs each distinct unknown key, and `REJECT` fails the query. Removing the key restores `NONE`; invalid values are logged and leave the current policy in place. This is not read from `broker.conf`. See [SQL option policies](../../build-with-pinot/querying-and-sql/query-execution-controls/query-options.md#sql-option-policies). |
| pinot.broker.query.option.legacySyntaxMode | ALLOW | Live cluster policy for the legacy `OPTION(key=value)` suffix on all statement types: `ALLOW` applies it, `IGNORE` strips and drops its options, and `REJECT` fails with guidance to use `SET`. Removing the key restores `ALLOW`; invalid values are logged and leave the current policy in place. This is not read from `broker.conf`. See [SQL option policies](../../build-with-pinot/querying-and-sql/query-execution-controls/query-options.md#sql-option-policies). |
