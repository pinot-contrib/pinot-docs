...
| enforce.pool.based.assignment | false | When `true`, the controller validates table configs to require both pool-based instance assignment and replica-group-based assignment in `instanceAssignmentConfigMap`. Dimension tables and tables that reuse pre-existing `instancePartitionsMap` assignments are excluded from this check. |
...
| controller.ingestFromURI.allowLocalFileSystem                | false                                                                    | When `true`, the `/ingestFromURI` controller API may read controller-local files through `file:///...` URIs or `LocalPinotFS`. Leave this `false` unless you explicitly trust controller-local ingestion inputs.                                       |
| controller.lineage.exclusive.delete.enabled                  | true                                                                     | When `true`, Pinot keeps segments that are still locked by a live lineage entry on the lineage-managed cleanup path instead of deleting them through the generic segment delete flow. Set to `false` to restore the legacy delete behavior.             |
...
| pinot.controller.http.server.max.http.header.size            | unset (Grizzly default)                                                  | Maximum total HTTP header size in bytes accepted by the controller's Grizzly HTTP/HTTPS listeners. Set this when upstream proxies, auth gateways, or clients send larger headers than the Grizzly default allows.                                          |
| pinot.controller.http.server.max.request.headers             | unset (Grizzly default)                                                  | Maximum number of HTTP headers accepted by the controller's Grizzly HTTP/HTTPS listeners.                                                                                                                                                                   |
...
Pinot does not support enabling Helix batch message mode through `controller.conf`. Controllers now disable batch message mode on controller-managed IdealStates when those resources are created or updated, so any legacy `controller.enable.batch.message.mode` setting has no effect.
