...
| REALTIME_INGESTION_OFFSET_LAG | Per partition metric that measures the difference between the latest known upstream offset and the last consumed offset. Pinot emits and refreshes this gauge only when `pinot.server.instance.offset.lag.tracking.enable=true`. The refresh cadence is controlled by `pinot.server.instance.offset.lag.tracking.update.interval`. |  |
| LUCENE_INDEXING_DELAY_MS | Per partition metric for realtime Lucene text indexes that reports the maximum search-refresh delay, in milliseconds, across the text-indexed columns currently hosted for that partition on the server. Pinot resets the gauge to `0` when the partition has no pending Lucene-refresh backlog. |  |
...
| MAX_RECORDS_LAG | Per-table, per-partition controller gauge for the maximum records lag across consuming segments when `RealtimeConsumerMonitor` is enabled. Pinot emits this only for lag values it can calculate; partitions reported as `NOT_CALCULATED` are skipped. Streams that do not support offset lag do not emit this gauge. | Gauge |
| MAX_RECORD_AVAILABILITY_LAG_MS | Per-table, per-partition controller gauge for the maximum record-availability lag in milliseconds across consuming segments when `RealtimeConsumerMonitor` is enabled. Pinot emits this only for lag values it can calculate; partitions reported as `NOT_CALCULATED` are skipped. | Gauge |
