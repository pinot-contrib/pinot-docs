...
| `stream.[streamType].consumer.prop.auto.offset.reset` | Pinot's initial-offset criterion for a partition that has no stored consumption offset. Despite the key name, this value is not passed to the Kafka client and does not control recovery from an expired stored offset. | `smallest`, `largest`, period (`10d`, `4h30m`, etc.), or timestamp (for example, `2022-08-09T12:31:38.222Z`) |
...
| `auto.offset.reset` | Raw Kafka consumer property used when the consumer's requested offset no longer exists. Default: `earliest`, which resumes from the oldest record Kafka still retains. Set `latest` explicitly to skip to the log end instead. This is separate from `stream.kafka.consumer.prop.auto.offset.reset`, which chooses only the initial offset for a partition with no stored Pinot offset. | Kafka values `earliest`, `latest`, or `none`. |
